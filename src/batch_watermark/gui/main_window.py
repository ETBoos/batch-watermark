...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        hw_l.addWidget(self.hw_label)

        hw_l.addWidget(section_caption("显卡"))
        self.gpu_label = info_label()
        hw_l.addWidget(self.gpu_label)

        self.hw_encode_check = QCheckBox("优先使用显卡编码（失败自动回退 CPU）")
        self.hw_encode_check.setChecked(True)
        self.hw_encode_check.stateChanged.connect(lambda _s: self._refresh_gpu_encode_ui())
        hw_l.addWidget(self.hw_encode_check)

        hw_l.addWidget(section_caption("将使用的编码器"))
        self.encoder_label = info_label()
        self.encoder_label.setStyleSheet(
            "background: #f5f5f7; border: 1px solid #ddd; border-radius: 6px; padding: 8px;"
        )
        ...
        hw_l.addWidget(section_caption("CPU 利用率（同时处理条数）"))
        cpu_row = QHBoxLayout()
        ...
        self.cpu_value.setMinimumWidth(42)
        ...
        hw_l.addLayout(cpu_row)

        self.budget_hint = info_label()
        hw_l.addWidget(self.budget_hint)

        retry_row = QHBoxLayout()
        retry_row.addWidget(section_caption("最大重试次数"))
        self.max_retries = QSpinBox()
        self.max_retries.setRange(0, 10)
        self.max_retries.setValue(3)
        self.max_retries.setMaximumWidth(100)
        retry_row.addWidget(self.max_retries)
        retry_row.addStretch(1)
        hw_l.addLayout(retry_row)

        hw_l.addWidget(section_caption("ffmpeg"))
        self.ffmpeg_label = info_label()
        self.ffmpeg_label.setStyleSheet(
            "background: #f5f5f7; border: 1px solid #ddd; border-radius: 6px; padding: 8px;"
        )
        hw_l.addWidget(self.ffmpeg_label)
        ...
        right_layout.addWidget(section_caption("实时日志"))
        self.log_view = QTextEdit()
        self.log_view.setReadOnly(True)
        right_layout.addWidget(self.log_view, stretch=2)

        right_layout.addWidget(section_caption("失败列表"))
    ...
    def _make_opacity_spin(self) -> QDoubleSpinBox:
        w = QDoubleSpinBox()
        w.setRange(0.05, 1.0)
        w.setSingleStep(0.05)
        w.setValue(0.5)
    ...
    def _make_position_combo(self) -> QComboBox:
        w = QComboBox()
    ...
    def _make_margin_spin(self) -> QSpinBox:
        w = QSpinBox()
        w.setRange(0, 500)
        w.setValue(20)
        return w
